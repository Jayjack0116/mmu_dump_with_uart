...
static void mmu_dump_pages(unsigned int vaddr, unsigned int entry)
{
    ...
    if ((entry & 0x3) == 0x1) /* large page */
    {
        ...
        printf("-[0x%p] %s PA:0x%p NG:%d SH:%d UR:%d UW:%d PR:%d PW:%d XN:%d\n\r", vaddr, "Lg Page  ", paddr, !!ng, !!s, !!ur, !!uw, !!pr, !!pw, !!xn);
    }
...
static void mmu_dump_sections(unsigned int vaddr, unsigned int entry)
{
    ...
    if ((entry & 0x3) == 2) /* section or supersection */
    {
        ...
        printf("[0x%p] %s PA:0x%p NG:%d SH:%d UR:%d UW:%d PR:%d PW:%d XN:%d NS:%d DOM:%X\n\r\r", vaddr, ss ? "S-Section " : "Section   ", paddr, !!ng, !!s, !!ur, !!uw, !!pr, !!pw, !!xn, !!ns, domain);
    }
    else if ((entry & 0x3) == 1) /* page table */
    {
        ...
        printf("[0x%p] %s PA:0x%p VA:0x%p NS:%d DOM:%X\n\r", vaddr, "Page TBL  ", paddr, tbl, !!ns, domain);
        for (i = 0; i < 0x100; i++)
        {
...
int mmu_dump(void)
{
    ...
    printf("TTBR0: 0x%p, TTBR1: 0x%p, TTBCR: 0x%p\n\r", ttbr[0], ttbr[1], ttbcr);
    ...
    printf("TBBR0 (physical): 0x%p, (virtual): 0x%p\n\r", ttbr[0], i<<12);
    printf("TBBR1 (physical): 0x%p, (virtual): 0x%p\n\r", ttbr[1], i<<12);
    ...
    for (i = 0; i < (1 << 12 - n); i++)
    {
        ...
        printf("context id is %p\r\n", contextid);

        printf("  i  = %d , entry = %p  ", i,entry);
